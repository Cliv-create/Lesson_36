...
public:
	static int Random(int min, int max) {
		...
		static mt19937 gen(rd()); // генерация по алгоритму "вихрь мерсенна"
...
public:
	...
	template<size_t N>
	void SentenceComposition(string (&path_array)[N]) {
		srand(time(0));
		rand();
...
int main() {
	...
	// FileStreamsDemo::SentenceComposition(patharray);
